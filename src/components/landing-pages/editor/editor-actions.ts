...
export function renderLandingPageHtml(data: EditorData): string {
  ...
  const generatedCss = collectCssRules(normalized.blocks);
...
function collectCssRules(blocks: EditorBlock[]): string {
  const rules: string[] = [];
  ...
  function traverse(node: EditorBlock) {
    ...
    if (isSection) {
      ...
      rules.push(`#${node.id} { position: relative; width: 100%; height: ${height}px; overflow: hidden; }`);
    } else if (frame) {
    ...
    if (node.type === "columns" && node.props && Array.isArray(node.props.children)) {
      const cols = node.props.children as EditorBlock[][];
      cols.forEach((col) => col.forEach(traverse));
    }
  }
