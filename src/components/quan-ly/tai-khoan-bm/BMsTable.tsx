  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center p-12 min-h-[500px] w-full text-center">
        ...
        <h3 className="text-base font-bold text-gray-800 dark:text-white mb-1">Đang quét thông tin Business Manager...</h3>
                  ...
                  >
                    <td className="px-4 py-3.5 text-center">
                      <input
                        type="checkbox"
                        checked={isSelected}
                        onChange={(e) => onSelectRow(bm.id, e.target.checked)}
                        className="rounded text-blue-600 focus:ring-blue-500 w-4 h-4 cursor-pointer"
                      />
                    </td>
                    <td className="px-4 py-3.5 text-center">
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex flex-col text-left">
                        ...
                        <span className="text-[10px] text-gray-400 mt-0.5">BM ID: {bm.bmId}</span>
                      </div>
                    </td>
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-2 text-[10px] font-medium text-gray-400">
                        <div className="w-20 bg-gray-100 dark:bg-gray-800 rounded-full h-1.5 overflow-hidden">
                          <div
                            ...
                            style={{ width: bm.status === "ACTIVE" ? "100%" : bm.status === "DISABLED" ? "0%" : "50%" }}
                          />
                        </div>
                        <span>{bm.status === "ACTIVE" ? "100%" : bm.status === "DISABLED" ? "0%" : "50%"}</span>
                      </div>
                    </td>
                    {dataOptions.limit && (
                    ...
                    {dataOptions.page && (
                      <td className="px-4 py-3.5 text-center font-medium">{bm.pagesCount}</td>
                    )}
                    {dataOptions.partner && (
                      <td className="px-4 py-3.5 text-center font-medium">{bm.partnersCount}</td>
                    )}
                    {dataOptions.admin && (
                      <td className="px-4 py-3.5 text-center font-medium">{bm.adminsCount}</td>
                    )}
                    <td className="px-4 py-3.5 text-center">
