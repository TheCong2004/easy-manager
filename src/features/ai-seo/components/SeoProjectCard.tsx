...
export function SeoProjectCard({ project }: SeoProjectCardProps) {
  ...
  return (
    <div className="bg-white border border-slate-200/80 hover:border-slate-300 rounded-2xl p-6 shadow-sm hover:shadow-md transition duration-200 flex flex-col gap-4 text-slate-800">
      {/* Header Info */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <button
            ...
            className="text-slate-350 hover:text-amber-500 transition shrink-0"
          >
            <Star
              className={`w-5 h-5 ${
                project.isFavorite
          ...
          <div className="w-8 h-8 rounded-lg bg-slate-50 border border-slate-100 flex items-center justify-center shrink-0 overflow-hidden shadow-inner">
            <img
              src={faviconUrl}
          ...
          <div className="flex flex-col text-left min-w-0">
            <span className="font-extrabold text-sm text-slate-900 truncate tracking-tight">
              {project.hostname}
            </span>
            <span className="text-[9px] text-slate-400 font-extrabold uppercase tracking-wide leading-none mt-0.5">
...
function ProjectLandingPagesStats({ projectId }: { projectId: string }) {
  ...
  return (
    <div className="border-t border-slate-50 pt-3 flex flex-wrap items-center justify-between gap-2 text-xs">
      <div className="flex items-center gap-1.5 text-slate-500 font-medium">
        <Link2 className="w-4.5 h-4.5 text-slate-400" />
        <span>Landing Pages:</span>
        <span className="font-extrabold text-slate-900">{pagesCount} liên kết</span>
        {pagesCount > 0 && (
      ...
      <Link
        ...
        className="text-slate-900 hover:text-slate-700 text-xs font-black transition flex items-center gap-1 underline decoration-dotted underline-offset-2"
      >
